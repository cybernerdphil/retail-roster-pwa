/**
 * SERVICE WORKER — RETAIL ROSTER AVAILABILITY FORM
...
const CACHE_STATIC   = `static-v${VERSION}-retail`;
const CACHE_FONTS    = `font-v${VERSION}-retail`;
const CACHE_EXTERNAL = `external-v${VERSION}-retail`;
...
self.addEventListener('install', (event) => {
  console.log(`[SW] Installing Retail service worker v${VERSION}`);
...
self.addEventListener('activate', (event) => {
  console.log('[SW] Activating Retail service worker');

  event.waitUntil(
    caches.keys().then((cacheNames) => {
      return Promise.all(
        cacheNames.map((cacheName) => {
          if (!cacheName.includes(`v${VERSION}-retail`)) {
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    const title = data.title || 'Retail Roster';
    const options = {
      body: data.body || 'You have a new roster message from your store manager',
  ...
  event.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then((clientList) => {
      for (const client of clientList) {
        if (client.url.includes('/retail-roster-pwa/') && 'focus' in client) {
          return client.focus();
        }
      }
...
console.log(`[SW] Retail service worker v${VERSION} loaded`);
