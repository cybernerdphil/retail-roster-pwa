(() => {
  ...
  function buildAvailabilityTable() {
    ...
    DAYS.forEach(day => {
      ...
      // Start cell — default 08:00 (retail open)
      const startCell  = document.createElement('td');
      const startRow   = document.createElement('div');
      startRow.className = 'sc-time-cell';
      startRow.id = 'sc-srow-' + day.key;
      startRow.appendChild(buildTimeSelect('sc-start-' + day.key, '08:00'));
      ...
      // End cell — default 18:00 (retail close)
      ...
      endRow.appendChild(buildTimeSelect('sc-end-' + day.key, '18:00'));
  ...
  function buildPayload() {
    ...
    return {
      ...
      dept:           sanitize($('sc-dept').value, 80),
      availability:   av
    };
  }
  ...
  function makeFilename(name) {
    const slug = (name || 'team-member')
      .replace(/\s+/g, '-').toLowerCase().replace(/[^a-z0-9-]/g, '');
    return 'retail-avail-' + slug + '-' + new Date().toISOString().slice(0, 10) + '.json';
  }
  ...
  function openEmail(managerEmail, memberName, filename) {
    const subj = encodeURIComponent('Retail Roster Availability — ' + memberName);
    const body = encodeURIComponent(
      'Hi,\n\nPlease find my weekly availability submission attached as:\n    ' + filename +
      '\n\nTo import it into the Retail Roster, go to:\n    Backup → Import Availability → select the attached file\n\n' +
      'Submitted by: '  + memberName + '\nSubmitted at: ' + new Date().toLocaleString() +
      '\n\n— Sent via Retail Roster Availability Form'
    );
  ...
  async function handleShare() {
    ...
    try {
      await navigator.share({
        title: 'Retail Roster Availability',
        text:  'Weekly availability for ' + (payload.memberName || 'team member'),
        files: [file]
      });
